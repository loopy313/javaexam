...
지 않은 것은?d	//method1이 method2 호출. 12번째 줄
...
[8-3] 다음 중 오버라이딩이 잘못된 것은? (모두 고르시오)d,e //예외범위가 좁거나 같아야 한다.
...
[8-4] 다음과 같은 메서드가 있을 때, 예외를 잘못 처리한 것은? (모두 고르시오)c,d
...
public static void main(String[] args) {
method(true);	//try문 들어가서 처음 1번 출력->b가 true이므로 에외발생 catch문 이동 후 3출력 후 리턴되기전 finally진입 후 5출력 마지막으로 catch
				//문으로 돌아간 후 리턴.
method(false);	//try문 1 출력->b가 false이므로 2출력 후 finally진입 후 5출력 그리고 다음줄 6출력후 종료. 
				//결과:1->3->5->1->2->5->6
} // main
}
...
//method1() 호출 후, try문 에서 method2()호출, NullPointerException()발생 method2에 catch문이 없으몰 1로 되돌려서
//NullPointerException()관련 catch문을 찾음. 1에도 관련 catch문이 없으나 finally문이 있으므로 진입후 3출력되고 다시 method1의 호출부로 예외 되돌림. 
//main() 올라온 후, Exception catch문 진입 후, 5출력. 
//결과 : 3->5
 * 
...
//method의 매개변수가 true이므로 1출력 후, System.exit(0)이 실행되는데System.exit(0)은 securityException이 발생되지 않는 한 finally를 실행 시키지 않고
//프로그램을 종료시킨다. 
//결과값 : 1
...
class UnsupportedFunctionException extends RuntimeException{
	...
	UnsupportedFunctionException(String desc,int err){
		super("["+err+"]"+desc);
	}
...
//method2()가 예외를 발생시키고 처리를 위해method1()의 catch문 실행. 2출력후 다시 같은 예외를 발생시키고 method2()의 finally문에서 4출력후 method1()을 호출한
//main()의 catch문으로 이동. 마지막으로 7출력.
//결과값 : 2->4->7 
*/
	}
