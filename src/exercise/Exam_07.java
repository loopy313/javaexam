...
-부모 클래스로부터 상속받은 메스드의 내용을 변경하는 것.
-자식 클래스에서 부모 클래스로부터 상속받은 메스드의 내용을 자신에 맞게 변경할 필요가
-있을때 사용한다.
...
c. 접근 제어자는 조상의 메서드보다 좁은 범위로만 변경할 수 있다.-->범위가 같거나 넓은 범위로 변경
d. 조상의 메서드보다 더 많은 수의 예외를 선언할 수 있다.-->조상 클래스의 메서드보다 많이 선언할 수 없다.
...
class Tv extends Product {
	Tv() {}	/자식 클래스의 인스턴스 생성자 자식클래스의 생성자를 올라가 부모클래스의 생성자가 실행되는데
			//현재 부모클래스의 생성자는 매개변수를 가지고 있는데 참조할 매개변수가 자식 클래스에 존재 하지않기때문에 에러를 피하기 위해서는
			//부모클래스에 매개변수가 없는 생성자를 만들면된다.
	public String toString() {
		return "Tv";
	}
}
...
//Child()->Child(int x)->Parent()->Parent(int x) 실행결과:200
...
옳지 않은 것은? (모두 고르시오)b,c
	a. 지역변수 - 값을 변경할 수 없다.
	b. 클래스 - 상속을 통해 클래스에 새로운 멤버를 추가할 수 없다.	//상속할 수 없다.
	c. 메서드 - 오버로딩을 할 수 없다.						//오버라이딩 할수 없다.
	d. 멤버변수 - 값을 변경할 수 없다.
...
[7-12] 다음 중 접근 제어자에 대한 설명으로 옳지 않은 것은? (모두 고르시오)c,d
...
[7-15] 클래스가 다음과 같이 정의되어 있을 때, 형변환을 올바르게 하지 않은 것은?c,e
		(모두 고르시오.)
class Unit {}
...
//Movable을 상속받는 클래스,Movable 익명클래스
