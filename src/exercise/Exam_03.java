...
/*
	...
	System.out.println(1 + x << 33);				//+가 <<보다 우선순위가 높으므로 (1+2)>>33=6; 
	System.out.println(y >= 5 || x < 0 && x > 2);	//true||false&&false =>true
	System.out.println(y += 10 - x++);				//x++는(int temp=x; x+=1; return temp;) 이므로 5+=(10-2)=13
	System.out.println(x+=2);						//앞에서 x++연산을 했기때문에 이번 연산에서 x=3 그러므로 x=3+2=5;
	System.out.println( !('A' <= c && c <='Z') );	//!(true && true)=false;
	System.out.println('C'-c);						//'C'-'A' == 67-65 이므로 2;
	System.out.println('5'-'0');					//위와같이 5;
	System.out.println(c+1);						//65+1=66;
	System.out.println(++c);						//전위증가연산자 이고 단항연산이므로 'B' 
	System.out.println(c++);						//이전 함수에서 c값이 증가 되었으므로 'B'
	}
	}
*/
...
/*
	...
	float f = 3 / 2;				//float f = (float) 3 / 2;
	long l = 3000 * 3000 * 3000;	//long l = 3000L*3000L*3000L
	float f2 = 0.1f;				
...
/*
	...
	public static void main(String[] args) {
	char ch = 'A';
	char lowerCase = (((char)ch+32)>='a'&&((char)ch+32)<='z') ? ((char)(ch+32)) : ch;
	System.out.println("ch:"+ch);
