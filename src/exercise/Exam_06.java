...
float getAverage(){
	return Math.round((kor+eng+math)/3.0*10)/(float)10;
}
}
[실행결과]
이름:홍길동
...
//static을 붙이지 않은 변수는 각 객체에 종속적이므로 모든 병사의 weapon과 armor이
//공유 되기 위해서는 두 변수에 static 선언을 하여 클래스변수로 만들어 줘야 한다.

[6-10] 다음 중 생성자에 대한 설명으로 옳지 않은 것은? (모두 고르시오)b,c,e
a. 모든 생성자의 이름은 클래스의 이름과 동일해야한다.
b. 생성자는 객체를 생성하기 위한 것이다.//인스턴스 변수 초기화
c. 클래스에는 생성자가 반드시 하나 이상 있어야 한다.//생성자 샹략시 암시적으로 생성됨
d. 생성자가 없는 클래스는 컴파일러가 기본 생성자를 추가한다.
e. 생성자는 오버로딩 할 수 없다.//오버로딩 가능

[6-11] 다음 중 this에 대한 설명으로 맞지 않은 것은? (모두 고르시오)b
a. 객체 자신을 가리키는 참조변수이다.
b. 클래스 내에서라면 어디서든 사용할 수 있다.//static 함수내에서는 사용 불가.
...
c. 리턴타입이 달라야 한다.		//리턴타입의 변경만으로는 오버로딩 불가.
d. 매개변수의 이름이 달라야 한다.	//매개변수의 자료형이나 개수를 변경해야한다.
...
[6-14] 다음 중 초기화에 대한 설명으로 옳지 않은 것은? (모두 고르시오)e
...
e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다.	//스택영역
...
static int cv = 20;
int iv2 = cv;
static int cv2 = iv; // 라인 A	-인스턴스화가 필요한 iv변수는 클래스 변수에 대입할수 없다.
static void staticMethod1() {
System.out.println(cv);
System.out.println(iv); // 라인 B	-클래스멤버함수에는 인스턴스 변수를 사용할 수 없다.
}
...
staticMethod1();
instanceMethod1(); // 라인 D		-클래스멤버함수에는 인스턴스함수를 사용할 수 없다.
}
